...
def get_copys_sum_510(module_name: str, sub_module_name_list: list[str], copy_list: list[str], ethics_db_names: list[str])->list[str]:
    ...
    if DB_FILE_NAMES:
        #ユースケース5-6
        CopyFileKubunList = get_ronri_db_copyku_yamamura(DB_FILE_NAMES, copy_list)
    else:
    ...
    if copy_file_name_list:
        for copy_file_name in copy_file_name_list:
            #ユースケース5-8
            first_paramate_list.append(get_oya_komoku_yamamura(module_name_list, copy_file_name))
    else:
    ...
    if first_paramate_list:
        for first_paramate in first_paramate_list:
            #ユースケース5-9
            second_paramate_list.append(get_second_paramate_yamamura(module_name_list, first_paramate))
            # output
                # second_paramate_list　MMMをしている親項目名の第二引数

    if second_paramate_list:
        #ユースケース5-10
        db_copy = get_db_copy_in_pgm(module_name_list,second_paramate_list)
        if db_copy:
            return db_copy
        else:
            db_copy = get_db_copy_in_copyku(second_paramate_list,copy_list)
            if db_copy:
                return db_copy
            else:
                return "dbが取得できない" 
